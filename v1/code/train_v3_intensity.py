...
def load_data(csv_path: Path):
    ...
    # Adding Target_Intensity as the 4th feature
...
def train_model(name: str, dataset: TensorDataset):
    print(f"\n--- Training {name.upper()} Model (with Intensity) ---")
    ...
    metrics = {
        ...
        "weights": {
            "semantic": model.slp.weight[0][0].item(),
            "emotional": model.slp.weight[0][1].item(),
            "temporal": model.slp.weight[0][2].item(),
            "intensity": model.slp.weight[0][3].item(),
            "bias": model.slp.bias[0].item()
        }
    }
    
    print(f"Final Weights for {name}:")
    for k, v in metrics["weights"].items():
        print(f"  {k}: {v:.4f}")
...
def main():
    ...
    c_model, c_hist, c_metrics = train_model("causal_v3", causal_ds)
    results["causal_v3"] = {"history": c_hist, "metrics": c_metrics}
    torch.save(c_model.state_dict(), "v1/artifacts/pretrained/als_causal_v3.pt")
    
    # Task 2: Affective
    affective_ds = load_data(affective_csv)
    a_model, a_hist, a_metrics = train_model("affective_v3", affective_ds)
    results["affective_v3"] = {"history": a_hist, "metrics": a_metrics}
    torch.save(a_model.state_dict(), "v1/artifacts/pretrained/als_affective_v3.pt")
    ...
    b_model, b_hist, b_metrics = train_model("combined_v3", both_ds)
    results["combined_v3"] = {"history": b_hist, "metrics": b_metrics}
    torch.save(b_model.state_dict(), "v1/artifacts/pretrained/als_combined_v3.pt")
    ...
    with open("v1/artifacts/pretrained/v3_intensity_metrics.json", "w") as f:
        json.dump(final_output, f, indent=4)
    print("\nAll V3 models trained and results saved.")
