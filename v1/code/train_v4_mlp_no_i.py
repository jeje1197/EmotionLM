...
def load_data(csv_path: Path):
    ...
    # Intensity-Blind: ONLY Semantic, Emotional, and Temporal
...
def train_model(name: str, dataset: TensorDataset):
    print(f"\n--- Training {name.upper()} MLP No-I Model ---")
    ...
    model = ALSModelMLPNoI(hidden_dim=CONFIG["hidden_dim"])
    ...
    for epoch in range(CONFIG["num_epochs"]):
        ...
        history.append({"epoch": epoch + 1, "train_loss": train_loss, "val_loss": val_loss, "val_accuracy": acc})
...
def main():
    ...
    c_model, _, c_metrics = train_model("causal_no_i", causal_ds)
    results["causal_no_i"] = {"metrics": c_metrics}
    torch.save(c_model.state_dict(), "v1/artifacts/pretrained/als_causal_no_i.pt")
    
    # Task 2: Affective
    affective_ds = load_data(affective_csv)
    a_model, _, a_metrics = train_model("affective_no_i", affective_ds)
    results["affective_no_i"] = {"metrics": a_metrics}
    torch.save(a_model.state_dict(), "v1/artifacts/pretrained/als_affective_no_i.pt")
    ...
    # Combined training
    b_model, _, b_metrics = train_model("combined_no_i", both_ds)
    results["combined_no_i"] = {"metrics": b_metrics}
    torch.save(b_model.state_dict(), "v1/artifacts/pretrained/als_combined_no_i.pt")
    
    with open("v1/artifacts/pretrained/no_i_metrics.json", "w") as f:
        json.dump(results, f, indent=4)
    print("\nAll Intensity-Blind models trained.")
