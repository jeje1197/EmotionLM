...
def load_data(csv_path: Path):
    ...
    features = ["Semantic_Similarity", "Emotional_Alignment", "Time_Closeness"]
...
def main():
    ...
    c_model, c_hist, c_metrics = train_model("causal", causal_ds)
    results["causal"] = {"history": c_hist, "metrics": c_metrics}
    torch.save(c_model.state_dict(), "v1/artifacts/pretrained/als_causal_v2.pt")
    
    # Task 2: Affective
    affective_ds = load_data(affective_csv)
    a_model, a_hist, a_metrics = train_model("affective", affective_ds)
    results["affective"] = {"history": a_hist, "metrics": a_metrics}
    torch.save(a_model.state_dict(), "v1/artifacts/pretrained/als_affective_v2.pt")
    ...
    b_model, b_hist, b_metrics = train_model("combined", both_ds)
    results["both"] = {"history": b_hist, "metrics": b_metrics}
    torch.save(b_model.state_dict(), "v1/artifacts/pretrained/als_combined_v2.pt")
    ...
    with open("v1/artifacts/pretrained/v2_training_metrics.json", "w") as f:
        json.dump(final_output, f, indent=4)
    print("\nAll models trained and results saved to v1/artifacts/pretrained/")
